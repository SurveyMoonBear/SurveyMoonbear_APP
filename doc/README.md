# Table of contents

## Welcome user
- [Question Type](user_guide/question_type.md)

## Welcome developer
- [Getting Start](getting_start.md)

### Architecture

* [Architecture Intro](architecture/architecture-intro.md)
* [Infrastructure](architecture/infrastructure.md)
* [Services](architecture/services.md)
* [Client-side browser application](architecture/client-side-browser-application.md)

### Google

* [Applying Google OAuth Verification](google/applying-google-oauth-verification.md)
* [Google Drive API](google/google-drive-api.md)

### AWS

* [SQS](aws/sqs.md)

### Heroku
* [Test on Heroku new stack](heroku/test-on-heroku-new-stack.md)
* [pgAdmin Remote Connection](heroku/pgAdmin-remote-connection.md)
