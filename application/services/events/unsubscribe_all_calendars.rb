...
module SurveyMoonbear
  module Service
    ...
    # Usage: Service::UnsubscribeAllCalendars.new.call(config: <config>, current_account: {}, study_id:)
    class UnsubscribeAllCalendars
      include Dry::Transaction
      include Dry::Monads

      step :get_all_participants
      step :unsubscribe_all_calendars
      ...
      # input { ... }
      def unsubscribe_all_calendars(input)
        input[:participants].map do |participant|
          next if participant.act_status == 'unsubscribe'
        ...
        Failure('Failed to unsubscribe all participants\' calendars.')
      end
    end
  end
end
